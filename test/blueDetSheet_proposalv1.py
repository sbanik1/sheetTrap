# -*- coding: utf-8 -*-
"""
This code evaluates the Blue Detuned Sheet TEM01 pattern imprinted onto the atoms.
...
# Generating Data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[E2, X2, Y2] = OE.CylLensAction(E1,X1,Y1,k,f_cyY1,FocusingAxis = 'Y',FocusedAxis = 'None')
I2 = GB.BeamInt(E2,P)
# Generating Fits %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
I2 = I2.astype(float)
...
# Generating Data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[E3, X3, Y3] = OE.CylLensAction(E2,X2,Y2,k,f_cyY2,FocusingAxis = 'Y',FocusedAxis = 'Y')
I3 = GB.BeamInt(E3,P)
# Generating Fits %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
I3 = I3.astype(float)
p0 = [np.max(I3),0,0,w0,11*w0]
[params3,_] = optimize.curve_fit(GB.GaussBeamFit, [X3,Y3], I3.ravel(),p0 = p0)
I3_fit = GB.GaussBeamFit((X3,Y3),*params3).reshape(Ny,Nx)
plotGaussianBeam(3,X3,Y3,I3,I3_fit,params3,3,10)

# %% Cylindrical Lens f_cyX1  #################################################
# Generating Data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[E4, X4, Y4] = OE.CylLensAction(E3,X3,Y3,k,f_cyX1,FocusingAxis = 'X',FocusedAxis = 'Y')
I4 = GB.BeamInt(E4,P)
# Generating Fits %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
I4 = I4.astype(float)
p0 = [np.max(I4),0,0,10,11*w0]
[params4,_] = optimize.curve_fit(GB.GaussBeamFit, [X4,Y4], I4.ravel(),p0 = p0)
I4_fit = GB.GaussBeamFit((X4,Y4),*params4).reshape(Ny,Nx)
plotGaussianBeam(4,X4,Y4,I4,I4_fit,params4,0.1,10)

# %% Phase Plate ##############################################################
E5 = OE.PiPlateAction(E4,X4,Y4,PP_offset*10**3,PP_tilt*np.pi/180)
X5 = X4
Y5 = Y4
E5 = CircularAperture(E5,X5,Y5,20,0,PP_offset)
I5 = GB.BeamInt(E5,P)
figure = plotPhaseShiftedBeam(6,X5,Y5,I5,E5,0.1,5)

# %% Final Spherical lens f_sph  ########################################
# Generating Data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                             
[E6, X6, Y6] = OE.SphLensAction(E5,X5,Y5,k,f_sph,FocussedAxis ='XY')
I6 = GB.BeamInt(E6,P)
figure = plotGreenSheet(7,X6,Y6,I6,f_sph,5*10**3,3*10**1)
# cwd = os.getcwd()
# os.chdir(startUp.saveDir)
# figure.set_size_inches(10, 7)
...
# Generating Data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                             
[E7, X7, Y7] = OE.SphLensAction(E4,X4,Y4,k,f_sph,FocussedAxis ='None')
I7 = GB.BeamInt(E7,P)
# Generating Fits %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
I7 = I7.astype(float)
p0 = [np.max(I7),0,0,w0,10]
[params7,_] = optimize.curve_fit(GB.GaussBeamFit, [X7,Y7], I7.ravel(),p0 = p0)
I7_fit = GB.GaussBeamFit((X7,Y7),*params7).reshape(Ny,Nx)
plotGaussianBeam(5,X7,Y7,I7,I7_fit,params7,2,0.03)
